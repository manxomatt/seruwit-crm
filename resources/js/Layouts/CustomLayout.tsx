...
// Module to route mapping - use custom routes for custom panel
const moduleRouteMap: Record<string, { route: string; routePattern: string }> = {
    'pages': { route: 'custom.pages.index', routePattern: 'custom.pages.*' },
    'posts': { route: 'custom.posts.index', routePattern: 'custom.posts.*' },
    'carousels': { route: 'custom.carousels.index', routePattern: 'custom.carousels.*' },
    'media': { route: 'custom.media.index', routePattern: 'custom.media.*' },
    'analytics': { route: 'custom.analytics.index', routePattern: 'custom.analytics.*' },
    'settings': { route: 'custom.settings.index', routePattern: 'custom.settings.*' },
    'users': { route: 'custom.users.index', routePattern: 'custom.users.*' },
    'roles': { route: 'custom.roles.index', routePattern: 'custom.roles.*' },
    'live-updates': { route: 'custom.live-updates.index', routePattern: 'custom.live-updates.*' },
};
...
export default function CustomLayout({ header, children, roleName = 'Custom' }: Props) {
        ...
        const items: MenuItem[] = [
            {
                ...
                icon: <DashboardIcon />,
                current: route().current('custom.dashboard'),
                module: 'dashboard',
            },
        ];
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            {sidebarOpen && (
                <div className="fixed inset-0 z-40 lg:hidden">
                    ...
                    />
                    <div className="fixed inset-y-0 left-0 flex w-64 flex-col bg-gradient-to-b from-purple-700 to-purple-900">
                        <div className="flex h-16 items-center justify-between px-4">
                            <Link href="/" className="flex items-center">
                                <ApplicationLogo className="h-8 w-auto text-white" />
                                <span className="ml-2 text-xl font-bold text-white">{roleName} Panel</span>
                            </Link>
                            <button
                                type="button"
                        ...
                        <nav className="flex-1 space-y-1 px-2 py-4">
                            {navigation.map((item) => (
                                ...
                                >
                                    <span className={`mr-3 ${item.current ? 'text-white' : 'text-purple-300 group-hover:text-white'}`}>
                                        {item.icon}
                                    </span>
                                    {item.name}
                                </Link>
                        ...
                        <div className="border-t border-purple-600 p-4">
                            <Link href={route('profile.edit')} className="flex items-center hover:opacity-80 transition-opacity">
                                ...
                                <div className="ml-3">
                                    ...
                                    <p className="text-xs text-purple-200">{user?.email || 'user@example.com'}</p>
                                </div>
                            </Link>
                        </div>
                    </div>
            ...
            <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
                <div className="flex min-h-0 flex-1 flex-col bg-gradient-to-b from-purple-700 to-purple-900">
                    <div className="flex h-16 items-center px-4 border-b border-purple-600">
                        <Link href="/" className="flex items-center">
                            <ApplicationLogo className="h-8 w-auto text-white" />
                            <span className="ml-2 text-xl font-bold text-white">{roleName} Panel</span>
                        </Link>
