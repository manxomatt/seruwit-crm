...
class ModuleTierTest extends TestCase
{
    ...
    {
        foreach (['accounting', 'fleet', 'partners', 'products', 'document', 'maintenance', 'inventory', 'invoicing', 'purchasing', 'scoring'] as $key) {
            $this->assertSame(
    ...
    {
        foreach (['transportation', 'orders', 'billing', 'routing', 'promotions', 'bi'] as $key) {
            $this->assertSame(
                ModuleTier::Vertical,
                $this->registry->find($key)?->tier(),
