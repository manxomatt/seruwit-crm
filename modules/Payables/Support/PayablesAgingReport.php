...
/**
 * AP aging over open supplier bills (issued / partially paid with balance due).
 */
class PayablesAgingReport
{
    ...
    public static function build(?int $partnerId = null): array
    {
        ...
        foreach ($bills as $bill) {
            ...
            $anchor = $bill->due_date ?? $bill->bill_date;
    ...
    public static function byPartner(): Collection
    {
        ...
        return collect($report['rows'])
            ->filter(fn (array $row): bool => ($row['partner']['id'] ?? null) !== null)
