...
class RentalModule implements ModuleContract
{
    ...
    {
        return ['view', 'bookings', 'dispatch', 'damages', 'finance', 'rates', 'settings'];
    }
    ...
    public function routes(): void
    {
        ...
        Route::get('/rental/settings', [RentalSettingsController::class, 'index'])
            ->middleware('permission:rental,settings')
        ...
        // Tariff rates (CRUD)
        Route::get('/rental/rates', [RentalRateController::class, 'index'])->middleware('permission:rental,rates')->name('rental.rates.index');
        Route::get('/rental/rates/create', [RentalRateController::class, 'create'])->middleware('permission:rental,rates')->name('rental.rates.create');
        Route::post('/rental/rates/ai-generate', [RentalRateController::class, 'aiGenerate'])->middleware('permission:rental,rates')->name('rental.rates.ai-generate');
        Route::get('/rental/rates/suggest', [RentalRateController::class, 'suggest'])->middleware('permission:rental,rates')->name('rental.rates.suggest');
        Route::post('/rental/rates', [RentalRateController::class, 'store'])->middleware('permission:rental,rates')->name('rental.rates.store');
        Route::patch('/rental/rates/batch-status', [RentalRateController::class, 'batchUpdateStatus'])->middleware('permission:rental,rates')->name('rental.rates.batch-status');
        Route::post('/rental/rates/batch-destroy', [RentalRateController::class, 'batchDestroy'])->middleware('permission:rental,rates')->name('rental.rates.batch-destroy');
        Route::get('/rental/rates/{rate}/edit', [RentalRateController::class, 'edit'])->middleware('permission:rental,rates')->name('rental.rates.edit');
        Route::patch('/rental/rates/{rate}', [RentalRateController::class, 'update'])->middleware('permission:rental,rates')->name('rental.rates.update');
        Route::delete('/rental/rates/{rate}', [RentalRateController::class, 'destroy'])->middleware('permission:rental,rates')->name('rental.rates.destroy');
        ...
        // Rentals CRUD (Bookings)
        Route::get('/rental/list', [RentalController::class, 'index'])->middleware('permission:rental,view')->name('rental.index');
        Route::get('/rental/create', [RentalController::class, 'create'])->middleware('permission:rental,bookings')->name('rental.create');
        Route::post('/rental/walk-in-customers', [RentalController::class, 'storeWalkInCustomer'])->middleware('permission:rental,bookings')->name('rental.walk_in_customers.store');
        Route::post('/rental', [RentalController::class, 'store'])->middleware('permission:rental,bookings')->name('rental.store');
        Route::get('/rental/{rental}', [RentalController::class, 'show'])->middleware('permission:rental,view')->name('rental.show');
        Route::get('/rental/{rental}/edit', [RentalController::class, 'edit'])->middleware('permission:rental,bookings')->name('rental.edit');
        Route::get('/rental/{rental}/checkout', [RentalController::class, 'checkoutPage'])->middleware('permission:rental,dispatch')->name('rental.checkout_page');
        Route::get('/rental/{rental}/return', [RentalController::class, 'returnPage'])->middleware('permission:rental,dispatch')->name('rental.return_page');
        Route::patch('/rental/{rental}', [RentalController::class, 'update'])->middleware('permission:rental,bookings')->name('rental.update');
        Route::delete('/rental/{rental}', [RentalController::class, 'destroy'])->middleware('permission:rental,bookings')->name('rental.destroy');
        ...
        // Lifecycle actions
        Route::post('/rental/{rental}/confirm', [RentalActionController::class, 'confirm'])->middleware('permission:rental,finance')->name('rental.confirm');
        Route::post('/rental/{rental}/checkout', [RentalActionController::class, 'checkout'])->middleware('permission:rental,dispatch')->name('rental.checkout');
        Route::post('/rental/{rental}/return', [RentalActionController::class, 'return'])->middleware('permission:rental,dispatch')->name('rental.return');
        Route::post('/rental/{rental}/complete', [RentalActionController::class, 'complete'])->middleware('permission:rental,finance')->name('rental.complete');
        Route::post('/rental/{rental}/cancel', [RentalActionController::class, 'cancel'])->middleware('permission:rental,bookings')->name('rental.cancel');
        Route::post('/rental/{rental}/no-show', [RentalActionController::class, 'markNoShow'])->middleware('permission:rental,bookings')->name('rental.no_show');
        Route::post('/rental/{rental}/mark-fee-paid', [RentalActionController::class, 'markFeePaid'])->middleware('permission:rental,bookings')->name('rental.mark_fee_paid');
        Route::post('/rental/{rental}/extend', [RentalActionController::class, 'extend'])->middleware('permission:rental,bookings')->name('rental.extend');
        ...
        Route::post('/rental/{rental}/deposit-settle', [RentalActionController::class, 'settleDeposit'])->middleware('permission:rental,finance')->name('rental.deposit.settle');
        Route::post('/rental/{rental}/pay-invoices', [RentalActionController::class, 'payInvoices'])->middleware('permission:rental,finance')->name('rental.invoices.pay');
        Route::post('/rental/{rental}/approve-deposit-proof', [RentalActionController::class, 'approveDepositProof'])->middleware('permission:rental,finance')->name('rental.approve_deposit_proof');
        Route::post('/rental/{rental}/reject-deposit-proof', [RentalActionController::class, 'rejectDepositProof'])->middleware('permission:rental,finance')->name('rental.reject_deposit_proof');
        Route::post('/rental/{rental}/damages', [RentalActionController::class, 'storeDamage'])->middleware('permission:rental,damages')->name('rental.damages.store');
        ...
        Route::post('/rental/ai-pricing/analyze', [RentalAiPricingController::class, 'analyze'])
            ->middleware('permission:rental,rates')
