    ...
    return (
        <DynamicLayout header={t('canvassing.title')}>
            ...
            <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
                <div className="mb-4">
                    <Link href={prefixedRoute('canvassing.salespeople.index')} className="text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400">{t('canvassing.nav.salespeople')}</Link>
                </div>
                ...
                <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{t('canvassing.salespeople.contact')}</h3>
                        <dl className="space-y-1 text-sm">
                            <div><dt className="text-gray-400">{t('canvassing.salespeople.phone')}</dt><dd className="text-gray-900 dark:text-white">{salesperson.phone ?? '—'}</dd></div>
                            <div><dt className="text-gray-400">{t('canvassing.salespeople.email')}</dt><dd className="text-gray-900 dark:text-white">{salesperson.email ?? '—'}</dd></div>
                            <div><dt className="text-gray-400">{t('canvassing.salespeople.login')}</dt><dd className="text-gray-900 dark:text-white">{salesperson.user ? salesperson.user.email : t('canvassing.salespeople.no_account')}</dd></div>
                        </dl>
                    </div>

                    {/* This month */}
                    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{t('canvassing.salespeople.this_month')}</h3>
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                        <div className="mb-3 flex items-center justify-between">
                            <h3 className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                                {t('canvassing.salespeople.target_month', { month: t(`canvassing.months.${currentMonth}`), year: new Date().getFullYear() })}
                            </h3>
                        ...
                        {showTargetForm ? (
                            <div className="space-y-2">
                                ...
                                <div>
                                    <label className="block text-xs text-gray-500">{t('canvassing.salespeople.target_new_partners')}</label>
                                ...
                                <button onClick={saveTarget} className="w-full rounded-md bg-emerald-600 px-3 py-1.5 text-sm font-medium text-white hover:bg-emerald-700">{t('canvassing.salespeople.save_target')}</button>
                            </div>
                        ) : currentTarget ? (
                            <dl className="space-y-1 text-sm">
                                <div className="flex justify-between"><dt className="text-gray-400">{t('canvassing.salespeople.visits')}</dt><dd className="font-semibold text-gray-900 dark:text-white">{currentTarget.target_visits}</dd></div>
                                <div className="flex justify-between"><dt className="text-gray-400">{t('canvassing.salespeople.new_partners')}</dt><dd className="font-semibold text-gray-900 dark:text-white">{currentTarget.target_new_partners}</dd></div>
                            </dl>
                        ) : (
                ...
                <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <div className="border-b border-gray-200 px-4 py-3 dark:border-gray-700">
                        <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200">{t('canvassing.salespeople.visit_history')}</h2>
                    </div>
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {visits.data.length === 0 && (
                                <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-gray-400">{t('canvassing.salespeople.no_visits')}</td></tr>
                            )}
                            {visits.data.map((v) => {
                                const duration = v.checked_out_at
                                    ? t('canvassing.salespeople.duration_min', { count: Math.round((new Date(v.checked_out_at).getTime() - new Date(v.checked_in_at).getTime()) / 60000) })
                                ...
                                return (
                                    <tr key={v.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        ...
                                        <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{new Date(v.checked_in_at).toLocaleString(localeTag)}</td>
