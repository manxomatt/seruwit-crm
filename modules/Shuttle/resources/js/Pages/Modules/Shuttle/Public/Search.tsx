...
export default function Search({ brand, filters, corridors, departures, hold_ttl_minutes, gateway_available }: Props) {
    ...
    const submitSearch = (e: FormEvent) => {
        e.preventDefault();
        router.get(route('book.shuttle.search'), searchForm.data, { preserveState: true });
    };
                ...
                <form onSubmit={submitSearch} className="mb-6 grid gap-3 rounded-xl bg-white p-4 shadow-sm sm:grid-cols-3">
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => searchForm.setData('corridor_id', e.target.value)}
                        >
                            <option value="">Select…</option>
                            {corridors.map((c) => (
                                <option key={c.id} value={c.id}>
                                    {c.name} ({money(Number(c.base_fare))})
                                </option>
                            ))}
                        </select>
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            onChange={(e) => searchForm.setData('date', e.target.value)}
                        />
                    </div>
                            ...
                            <div>
                                ...
                                <div className="mt-1 flex gap-2">
                                    <input
                                        ...
                                        required
                                    />
                                    <button type="button" onClick={sendOtp} className="whitespace-nowrap rounded-md border px-3 text-sm">
                                        Send OTP
                                    </button>
                                </div>
