...
<body>
    <table class="header">
        <tr>
            ...
            <td>
                ...
                <div class="doc-meta">{{ $so->so_number }}</div>
                <div class="doc-meta">Tanggal: {{ $so->ordered_at?->format('d/m/Y') }}</div>
                <div class="doc-meta">Status: {{ $so->status }}</div>
            </td>
        </tr>
    </table>

    <table class="meta">
        <tr><td class="label">Customer</td><td>: {{ $so->partner?->name }} ({{ $so->partner?->code }})</td></tr>
        <tr><td class="label">Gudang</td><td>: {{ $so->warehouse?->name }}</td></tr>
    </table>
        ...
        <tbody>
            @foreach ($so->items as $item)
                <tr>
                    ...
                    <td class="qty">{{ number_format((float) $item->quantity_delivered, 2, ',', '.') }}</td>
    ...
    <p style="text-align:right;margin-top:10px;font-weight:bold;">
        Total: {{ $currencySymbol }} {{ number_format((float) $so->total_amount, 0, ',', '.') }}
    </p>
    ...
    <table class="sign">
        ...
        <tr>
            <td><div class="name">{{ $so->createdBy?->name ?? '—' }}</div></td>
            <td><div class="name">{{ $so->partner?->name }}</div></td>
        </tr>
    </table>
</body>
</html>
