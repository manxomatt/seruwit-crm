...
class StoreSalesOrderRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            'partner_id' => [
                'required',
                'integer',
                Rule::exists('partners', 'id')->where(fn ($query) => $query->where('customer_rank', '>', 0)),
            ],
    ...
    {
        return [
            'partner_id.exists' => __('sales.validation.customer_invalid'),
            'items.required' => __('sales.validation.items_required'),
            'items.min' => __('sales.validation.items_required'),
        ];
    }
            ...
            foreach ($this->input('items', []) as $index => $item) {
                ...
                if (! $belongs) {
                    $validator->errors()->add(
                        ...
                        __('sales.validation.packaging_mismatch'),
                    );
                }
            }
        });
