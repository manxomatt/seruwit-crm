...
export default function Index({ board, can }: Props): JSX.Element {
    ...
    const overdueShare = sharePercent(aging.overdue_amount, summary.open_ar);
    ...
    return (
        ...
        >
            <Head title={t('receivables.dashboard.title')} />
            ...
            <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <StatCard
                    ...
                    value={formatMoney(summary.open_ar)}
                    hint={t('receivables.dashboard.open_invoices', { count: summary.open_invoices })}
                    tone="sky"
                    icon={<IconInbox />}
                    href={prefixedRoute('receivables.aging.index')}
                />
                <StatCard
                    label={t('receivables.dashboard.overdue')}
                    value={formatMoney(aging.overdue_amount)}
                    hint={t('receivables.dashboard.overdue_hint', { count: aging.overdue_count })}
                    tone={aging.overdue_count > 0 ? 'rose' : 'slate'}
                    icon={<IconAlert />}
                    progress={overdueShare}
                    progressLabel={t('receivables.dashboard.of_open_ar')}
                    href={prefixedRoute('receivables.aging.index')}
                />
                <StatCard
                    label={t('receivables.payments.index.received_this_month')}
                    value={formatMoney(summary.posted_this_month)}
                    hint={t('receivables.dashboard.posted_payments', { count: summary.payments_posted })}
                    tone="emerald"
                    icon={<IconCash />}
                    href={prefixedRoute('receivables.payments.index')}
                />
                <StatCard
                    label={t('receivables.dashboard.needs_attention')}
                    value={alerts.attention}
                    hint={t('receivables.dashboard.needs_attention_hint')}
                    tone={alerts.attention > 0 ? 'amber' : 'slate'}
                    icon={<IconUsers />}
                    meta={[
                        { label: t('receivables.dashboard.overdue_short'), value: aging.overdue_count },
                        { label: t('receivables.dashboard.over_limit'), value: alerts.over_limit },
                        { label: t('receivables.dashboard.gateway_pending'), value: alerts.gateway_pending },
                    ]}
                />
            </div>
            ...
            <div className="rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-sm">
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <QuickAction
                        href={prefixedRoute('receivables.payments.index')}
                        title={t('receivables.nav.payments')}
                        description={t('receivables.dashboard.quick_payments')}
                    />
                    <QuickAction
                        href={prefixedRoute('receivables.aging.index')}
                        title={t('receivables.nav.aging')}
                        description={t('receivables.dashboard.quick_aging')}
                    />
                    <QuickAction
                        href={prefixedRoute('receivables.credit.index')}
                        title={t('receivables.nav.credit_limits')}
                        description={t('receivables.dashboard.quick_credit')}
                    />
                    <QuickAction
                        href={prefixedRoute('receivables.gateway.edit')}
                        title={t('receivables.nav.gateway')}
                        description={t('receivables.dashboard.quick_gateway')}
                    />
                </div>
            </div>
        </DynamicLayout>
